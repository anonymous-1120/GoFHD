...
cl <- makeCluster(min(numCores, 25))

# two types of standard deviation of y
y_sd = c(1, .1)
...
for (sd in y_sd){
  for (p in ps){
    result = foreach(ii=1:500, .combine = "rbind") %dopar% {
    set.seed(ii)
    ...
    mse_tr_mean <- mean((dat_tr[,1]-mean(dat_est[,1]))^2)
    ...
    network %>% keras :: fit(X_tr, dat_tr[,1],  epochs = 100)
